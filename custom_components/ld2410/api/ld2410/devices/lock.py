"""Library to handle connection with LD2410 Lock."""
...
from .device import LD2410EncryptedDevice, LD2410SequenceDevice

COMMAND_HEADER = "57"
COMMAND_LOCK_INFO = {
    LD2410Model.LOCK: f"{COMMAND_HEADER}0f4f8101",
    LD2410Model.LOCK_LITE: f"{COMMAND_HEADER}0f4f8101",
    LD2410Model.LOCK_PRO: f"{COMMAND_HEADER}0f4f8104",
    LD2410Model.LOCK_ULTRA: f"{COMMAND_HEADER}0f4f8107",
}
COMMAND_UNLOCK = {
    LD2410Model.LOCK: f"{COMMAND_HEADER}0f4e01011080",
    LD2410Model.LOCK_LITE: f"{COMMAND_HEADER}0f4e01011080",
    LD2410Model.LOCK_PRO: f"{COMMAND_HEADER}0f4e0101000080",
    LD2410Model.LOCK_ULTRA: f"{COMMAND_HEADER}0f4e0101000080",
}
COMMAND_UNLOCK_WITHOUT_UNLATCH = {
    LD2410Model.LOCK: f"{COMMAND_HEADER}0f4e010110a0",
    LD2410Model.LOCK_LITE: f"{COMMAND_HEADER}0f4e010110a0",
    LD2410Model.LOCK_PRO: f"{COMMAND_HEADER}0f4e01010000a0",
    LD2410Model.LOCK_ULTRA: f"{COMMAND_HEADER}0f4e01010000a0",
}
COMMAND_LOCK = {
    LD2410Model.LOCK: f"{COMMAND_HEADER}0f4e01011000",
    LD2410Model.LOCK_LITE: f"{COMMAND_HEADER}0f4e01011000",
    LD2410Model.LOCK_PRO: f"{COMMAND_HEADER}0f4e0101000000",
    LD2410Model.LOCK_ULTRA: f"{COMMAND_HEADER}0f4e0101000000",
}
COMMAND_ENABLE_NOTIFICATIONS = {
    LD2410Model.LOCK: f"{COMMAND_HEADER}0e01001e00008101",
    LD2410Model.LOCK_LITE: f"{COMMAND_HEADER}0e01001e00008101",
    LD2410Model.LOCK_PRO: f"{COMMAND_HEADER}0e01001e00008104",
    LD2410Model.LOCK_ULTRA: f"{COMMAND_HEADER}0e01001e00008107",
}
...
class LD2410Lock(LD2410SequenceDevice, LD2410EncryptedDevice):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if model not in (
            ...
            raise ValueError("initializing LD2410Lock with a non-lock model")
    ...
    @staticmethod
    def _parse_lock_data(data: bytes, model: LD2410Model) -> dict[str, Any]:
        if model == LD2410Model.LOCK:
            return {
        ...
        if model == LD2410Model.LOCK_LITE:
            return {
                "calibration": bool(data[0] & 0b10000000),
                "status": LockStatus((data[0] & 0b01110000) >> 4),
                "unlocked_alarm": bool(data[1] & 0b00010000),
